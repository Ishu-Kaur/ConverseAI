...
# Using a slightly larger sample as our model is smaller now
...
filters = '!"#$%&()*+,-/:;=?[\\]^_`{|}~\t\n'
# The tokenizer will now only consider the top 5000 words
...
# The vocab size is now capped by our limit
vocab_size = min(len(word_index) + 1, VOCAB_SIZE_LIMIT)
print(f"Vocabulary Size (limited to): {vocab_size}")
...
print("\nBuilding the lightweight model...")
encoder_inputs = Input(shape=(max_input_length,))
# The Embedding layer is now smaller due to the smaller vocab_size
...
# The Dense layer is now much smaller: (LSTM_UNITS, 5000) instead of (LSTM_UNITS, 20000)
...
# --- 9. Train ---
print("\nStarting FINAL model training (lightweight version)...")
model.fit(
...
print("Final lightweight trained model saved successfully to 'saved_models/seq2seq_model_final.keras'")